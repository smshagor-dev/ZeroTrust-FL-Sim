...
DASHBOARD_LOG = RUNTIME / "dashboard.log"
HOST = "127.0.0.1"
COLORS = {
    "SYSTEM": "\033[96m",
    "COORDINATOR": "\033[94m",
    "CPP-AGGREGATOR": "\033[95m",
    "WORKER-BENIGN": "\033[92m",
    "WORKER-ATTACKER": "\033[91m",
    "DASHBOARD": "\033[93m",
}
...
def append_dashboard_log(tag: str, name: str, msg: str) -> None:
    """Mirror managed-process output into the local dashboard log stream."""
    try:
        RUNTIME.mkdir(parents=True, exist_ok=True)
        with _LOG_LOCK:
            with DASHBOARD_LOG.open("a", encoding="utf-8") as handle:
                handle.write(
                    f"[{time.strftime('%H:%M:%S')}] [{tag}] {name} | {msg}\n"
                )
    except OSError:
        # Dashboard logging is supplemental and must never break orchestration.
        pass
...
def prepare(args: argparse.Namespace) -> str:
    ...
    log(
        "SYSTEM",
        f"Python={sys.version.split()[0]} Go={go} CMake={cmake} Git={git}",
    )

    dependency_probe = subprocess.run(
        [sys.executable, "-c", "import torch,grpc,numpy,psutil"],
    ...
    native_probe = subprocess.run(
        [
            ...
            "assert hasattr(n,'median_aggregate')",
        ],
        cwd=str(ROOT),
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
    ...
    run_checked(
        ...
        [
            ...
            "-c",
            "import zerotrust_fl_cpp as n;"
            "print('version',getattr(n,'__version__','?'),"
            "'OpenMP',getattr(n,'openmp_enabled',False),"
            "'SIMD',getattr(n,'simd_backend','scalar'),"
...
def prepare_dashboard(args: argparse.Namespace) -> list[str] | None:
    """Return the dashboard command, installing Node dependencies when allowed."""
    if not args.dashboard:
        return None

    package_json = FRONTEND / "package.json"
    if not package_json.exists():
        log("DASHBOARD", "frontend package is not present; skipped")
        return None

    npm = shutil.which("npm")
    if not npm:
        log("DASHBOARD", "npm not found on PATH; dashboard skipped")
        return None

    next_package = FRONTEND / "node_modules/next/package.json"
    react_package = FRONTEND / "node_modules/react/package.json"
    if not next_package.exists() or not react_package.exists():
        if not args.auto_install:
            log(
                "DASHBOARD",
                "frontend dependencies are missing and --no-auto-install is set; skipped",
            )
            return None
        run_checked(
            "DASHBOARD",
...
def worker_ids(args: argparse.Namespace) -> tuple[list[str], list[str]]:
    good = [
        f"benign-worker-{i}"
        for i in range(1, args.benign_workers + 1)
    ]
    bad = [
        f"malicious-worker-{i}"
        for i in range(1, args.malicious_workers + 1)
    ]
    if not good:
...
def ensure_certs(
    ...
    for node in nodes:
        required += [
            directory / f"{node}.crt",
            directory / f"{node}.key",
            directory / f"{node}.jwt",
        ]

    if args.force_cert_regen or not all(
        path.exists() and path.stat().st_size for path in required
    ):
...
class Manager:
    ...
    def start(
        ...
        *,
        ...
        process = subprocess.Popen(list(cmd), **kwargs)  # type: ignore[arg-type]
        child = Child(name, tag, process, final, optional)
    ...
    def wait(self) -> int:
        ...
        while not self.stop.wait(0.25):
            for child in self.children:
                return_code = child.p.poll()
                if return_code is None or child.p.pid in seen:
                    continue
                seen.add(child.p.pid)
                log(
                    child.tag,
    ...
    def shutdown(self) -> None:
        ...
        for child in reversed(
            [item for item in alive if item.p.poll() is None]
        ):
            self._signal(child.p, False)
...
def wait_port(
    ...
    while time.monotonic() < deadline:
        ...
        if child.p.poll() is not None:
            raise OrchestratorError(
                f"{child.name} exited before readiness"
            )
        if port_open(host, port):
            log(child.tag, f"ready on {host}:{port}")
            return
        time.sleep(0.2)
...
def validate(args: argparse.Namespace) -> None:
    ...
    f = args.malicious_workers
    if n <= 0 or f >= n:
        raise OrchestratorError(
            "training requires at least one benign client"
        )
    if args.aggregator in {"krum", "multi_krum"} and n < 2 * f + 3:
        raise OrchestratorError(
            f"{args.aggregator} needs n>=2f+3; got n={n}, f={f}. "
            "Use median or add clients."
        )
...
def trainer_cmd(args: argparse.Namespace) -> list[str]:
    ...
    if args.aggregator in {"krum", "multi_krum"}:
        command += ["--byzantine-f", str(args.malicious_workers)]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run Go + PyTorch workers + native C++ FL aggregation "
            "from one terminal."
        )
    )
    ...
    parser.add_argument("--shutdown-timeout", type=float, default=12)
    parser.add_argument(
        "--dashboard",
        action=argparse.BooleanOptionalAction,
        default=True,
    ...
    parser.add_argument(
        ...
        type=int,
        default=8080,
        help="local web dashboard port (kept for CLI compatibility)",
    )
...
def orchestrate(args: argparse.Namespace) -> int:
    ...
    go = prepare(args)
    good, bad = worker_ids(args)
    certs = ensure_certs(args, go, good + bad)
    if args.check_only:
        log(
            "SYSTEM",
            "prerequisites/native backend/PKI/topology ready",
        )
        return 0
    ...
    RUNTIME.mkdir(parents=True, exist_ok=True)
    try:
        DASHBOARD_LOG.unlink(missing_ok=True)
    except OSError:
        pass
    ...
    try:
        coordinator_command = [
            ...
            str(certs / "ca.crt"),
            "-jwt-public-key",
            str(certs / "jwt_signing_public.pem"),
            "-trust-domain",
            "zerotrust-fl.local",
            "-token-issuer",
            "zerotrust-fl-sim",
        ...
        if args.network_workers:
            ...
            specs = [
                (node, "WORKER-BENIGN", "none", 1000 + index)
                for index, node in enumerate(good)
            ] + [
                (
                    node,
                    "WORKER-ATTACKER",
                ...
                for index, node in enumerate(bad)
            ]

            for node, tag, attack, seed in specs:
                health_file = health_dir / f"{node}.ready"
                ...
                while (
                    not health_file.exists()
                    and time.monotonic() < deadline
                ):
                    if child.p.poll() is not None:
                        raise OrchestratorError(
                            f"{node} failed registration"
                        )
                    time.sleep(0.2)
                if not health_file.exists():
                    raise OrchestratorError(
                        f"timeout registering {node}"
                    )
        ...
        if dashboard_command:
            ...
            log(
                "DASHBOARD",
                f"web dashboard: http://{HOST}:{args.websocket_port}",
            )

        log(
            "CPP-AGGREGATOR",
            f"training {len(good)} benign + {len(bad)} attacker(s), "
            f"native {args.aggregator}, rounds={args.rounds}",
        )
        training = manager.start(
            "federated-training",
            "CPP-AGGREGATOR",
            trainer_cmd(args),
            final=True,
        ...
        time.sleep(0.5)
        if training.p.poll() is not None:
            raise OrchestratorError(
                f"training exited immediately: {training.p.returncode}"
            )
        log(
            "SYSTEM",
...
def main() -> int:
    try:
        return orchestrate(parse_args())
    except KeyboardInterrupt:
        log("SYSTEM", "interrupted")
        return 130
    except OrchestratorError as exc:
        log("SYSTEM", f"fatal: {exc}")
        return 1
